...
//        $sql = $sql." and due_date > ".date('Y-m-d H:i:s');
        ...
        if ($view) {
            ...
            $sql = $sql." order by sort DESC,juli asc,paid_at DESC";//created_at
        }
//        if (\request('lat')) {
...
//            if ($cityPartner = CityPartner::where('in_city',$request->district)->where('is_partners','>',1)->first()) {
            ...
            if ($cityPartner = CityPartner::where('in_city',$request->district)->where('market',$request->market)->where('is_partners','>',1)->whereNotNull('paid_at')->first()) {
                ...
                Log::info([
                    ...
                    'commissions'=>Setting::where('key', 'city_shop_fee')->value('value')?:0,//bcmul($rate,$amount,4),// 佣金
                    'rate'=>0,// 比例
...
//        }
        if (config('app.city') == 1) {
            ...
            $shopComment = ShopComment::where('shop_id', $id)->whereNull('parent_reply_id')->orderBy('created_at', 'desc')->where('id','>','155')->paginate();
        }
        // 浏览记录贴
...
//            }
            ...
            $result = $this->app->order->unify([
                ...
                'notify_url' => config('app.app_pay_url')."shop_wechat_notify", // 支付结果通知网址，如果不设置则会使用配置里的默认地址
...
//                    }
                    ...
                    if ($cityPartner = ShopCommission::where('shop_id',$order->id)->first()) {
                        CityPartner::where('in_city','like',$cityPartner->district.'%')->where('is_partners','>',1)->whereNotNull('paid_at')->where('market',$cityPartner->market)->increment('balance',$cityPartner->commissions);
